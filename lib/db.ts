...
function openDb(): Database.Database {
  ...
  if (!has("banner_color")) {
    db.exec("ALTER TABLE users ADD COLUMN banner_color TEXT NOT NULL DEFAULT '#8b5cf6'");
  }
  if (!has("accent_color")) {
    db.exec("ALTER TABLE users ADD COLUMN accent_color TEXT NOT NULL DEFAULT '#a78bfa'");
  }
  if (!has("bio")) {
    db.exec("ALTER TABLE users ADD COLUMN bio TEXT NOT NULL DEFAULT ''");
  }
  if (!has("avatar_focal_x")) {
    db.exec("ALTER TABLE users ADD COLUMN avatar_focal_x REAL NOT NULL DEFAULT 50");
  }
  if (!has("avatar_focal_y")) {
    db.exec("ALTER TABLE users ADD COLUMN avatar_focal_y REAL NOT NULL DEFAULT 50");
  }
  if (!has("avatar_zoom")) {
...
// Proxy — leniwie otwiera bazę przy pierwszym użyciu
